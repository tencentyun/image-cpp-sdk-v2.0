...
{
    sha_update(&m_sha, (SHA_BYTE*)data, size);
}
...
string Sha1::hexdigest()
{
    ...
    unsigned char *digest = (unsigned char *)m_sha.digest;

    stringstream ss;
    for(int i = 0; i < SHA_DIGESTSIZE; i++)
    {
        ss<<hex_digits[(digest[i] >> 4) & 0x0f];
        ss<<hex_digits[digest[i] & 0x0f];
    }
...
string Sha1::final()
{
    ...
    sha_final((unsigned char *)key, &m_sha);
    
    char out[64] = {0};
    sha_output((unsigned char *)key, (unsigned char*)out);
...
void sha_stream(unsigned char digest[20], SHA_INFO *sha_info, FILE *fin, int isize)
{
    int i;
    ...
    else
    {
        while ((i = fread(data, 1, BLOCK_SIZE, fin)) > 0) {
            ...
            {
                memset(data+i+isize,0,-isize);
                if (isize!=0) fseek(fin,isize,SEEK_CUR);
                sha_update(sha_info, data, i+isize);
                break;
            }
        }
    }
...
void sha_output(unsigned char digest[20], unsigned char output[40])
{
    int i=0;
    for(i=0;i<20;i++)
